...
테스트는 외부 상태에 따라ㅏ 성공하기도 하고 실패하기도 해서는 안된다.
...
```java {9,17} title="UserDaoTest.java"
...
```java {15,18,21,24} title="UserDaoTest.java"
public class UserDaoTest {
...
```java {8-9,18-20,22-24} title="UserDaoTest.java"
public class UserDaoTest {
...
| -                                           | 단계         | 내용                       | 코드 |
|---------------------------------------------|------------|--------------------------|---|
| 조건                                          | 어떤 조건을 가지고 | 가져올 사용자 정보가 존재하지 않는 경우에  |dao.d이eteAII( ); <br/>assertThat(dao.getCount(), is(0));|
